...
public class AxeBox implements IAxe{
	 ...
	 * by other OpenGL2 tokens (GL_POLYGON_TOKEN, GL_LINE_TOKEN, etc), or not.
	 ...
	 * is called with GL2.GL_3D_COLOR as second argument, i.e. vertices are made of
	...
	protected boolean[] getEmptyTokens(int ntokens, float[] buffer, int size){
		...
		while(count>0){
            ...
            if (token_type == GL2.GL_PASS_THROUGH_TOKEN) { // can't use switch cause we have floats
                ...
                if(printout)
                	System.out.println("GL2.GL_PASS_THROUGH_TOKEN: " + passthrough_value);
            ...
            // Other cases: just consume buffer
            	...
            	if(printout){
            		System.out.println(" GL2.GL_POINT_TOKEN");
            	...
            	if(printout){
            		System.out.println(" GL2.GL_LINE_TOKEN ");
            		count = print3DcolorVertex(size, count, buffer);
            		count = print3DcolorVertex(size, count, buffer);
            	}else{
            		count = count - 14;
            	...
            	if(printout){
            		System.out.println(" GL2.GL_LINE_RESET_TOKEN ");
            		count = print3DcolorVertex(size, count, buffer);
            		count = print3DcolorVertex(size, count, buffer);
            	}else{
            		count = count - 14;
            	...
            	if(printout)
            		System.out.println(" GL2.GL_POLYGON_TOKEN: " + n + " vertices");
