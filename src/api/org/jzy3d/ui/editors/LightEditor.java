...
public class LightEditor extends JPanel{
	...
	public LightEditor(Chart chart){
		...
		positionControl = new Coord3dEditor("Position", 
				box.getXRange().createEnlarge(2), 
				box.getYRange().createEnlarge(2), 
				box.getZRange().createEnlarge(2));
	...
	protected void registerColorControl(ColorEditor colorControl, final Color color){
		...
		/*final JSlider slider3 = colorControl.getSlider(3);
		slider3.setValue( (int)color.a * slider3.getMaximum() );
		slider3.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				color.a = getPercent(slider3);
				chart.render();
			}
		});*/
	}
